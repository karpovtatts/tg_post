...
async def get_prompts(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Ошибка при получении списка промптов"
        )
    ...
    try:
        ...
        if existing:
            raise HTTPException(
                ...
                detail="Промпт с таким tg_message_id уже существует"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Ошибка при создании промпта"
        )
...
async def add_tag_to_prompt(
    ...
    if not db_prompt:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Промпт или тег не найден"
        )
